...
# --- Pose canonica: el tornillo alineado siempre queda igual ---
...
# ===========================================================================
# PREPROCESAMIENTO: cargar, segmentar y alinear (esta parte funciona muy bien)
# ===========================================================================

def cargar_imagen(ruta):
    # Devuelve la imagen en color (para mostrar) y en gris (para trabajar).
...
def segmentar(img_gris):
    # Separa el tornillo del fondo con bordes (Canny). Mejor que Otsu porque
    # la sombra del fondo no tiene borde nitido y queda afuera.
    suave = cv2.GaussianBlur(img_gris, (3, 3), 0)
    bordes = cv2.Canny(suave, 40, 90)
    ee = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    bordes = cv2.dilate(bordes, ee, iterations=2)            # cerramos el contorno
    ...
    return cv2.erode(mascara, ee, iterations=2)             # volvemos al tamano real
...
def alinear(img_gris, mascara):
    ...
    M[1, 2] += h / 2 - cy_rot                                # eje -> fila central
...
# ===========================================================================
# LOS 3 DETECTORES (cada uno busca una familia de defectos)
# ===========================================================================
...
def detectar_intensidad(gris_a, masc_a, media, std):
    ...
    # raro estadistica (z) Y visualmente (niveles de gris)
    defecto = ((np.abs(z) > 2.5) & (np.abs(dif) > 20) & interna_bool).astype(np.uint8) * 255
    # cierre: suelda las estrias finas del rayon en una mancha
...
def detectar_forma(masc_a, nucleo, exterior):
    ...
    exterior = donde NUNCA hay metal en los sanos  -> si sobra, defecto.
    """
    ee = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (11, 11))
    nuc, ext = cv2.erode(nucleo, ee), cv2.erode(exterior, ee)   # margen de seguridad
    falta = cv2.bitwise_and(nuc, cv2.bitwise_not(masc_a))       # falta metal
    sobra = cv2.bitwise_and(masc_a, ext)                        # sobra metal
...
def detectar_perfil(masc_a, bandas):
    ...
    sup = np.where(hay, FILA_EJE - binaria.argmax(axis=0), 0.0).astype(np.float32)
    inf = np.where(hay, (h - 1 - binaria[::-1].argmax(axis=0)) - FILA_EJE, 0.0).astype(np.float32)
    ...
    ee1d = cv2.getStructuringElement(cv2.MORPH_RECT, (63, 1))
    perfiles = {"cresta_sup": cv2.dilate(sup.reshape(1, -1), ee1d).ravel(),
                "valle_sup": cv2.erode(sup.reshape(1, -1), ee1d).ravel(),
    ...
    for nombre, perfil in perfiles.items():
        bmin = bandas[f"{nombre}_min"] - 2                  # banda de los sanos + holgura
        bmax = bandas[f"{nombre}_max"] + 2
        fuera = (perfil > bmax) | (perfil < bmin)
        fuera[:490] = False                                 # solo rosca + punta
        fuera[990:] = False
        for x in np.where(fuera)[0]:
            radio = int(perfil[x])
